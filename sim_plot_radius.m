...
elevation_range = [-0.3715,-0.7804; 0.1608,-0.4636;...
-0.4438,-0.7706;0.2702,-0.2449;...
-0.4640,-0.7610;0.3101,-0.16514;...
-0.4699,-0.7515;0.3305,-0.12435;...
-0.4703, -0.7424; 0.3429, -0.0997];
N_inex = 1e2;%for electrical tilt
Nite = 1e2;
capacity = zeros(length(Rmax_store), 4);
...
for signalpo_n = 1 : length(Rmax_store)
    Rmax = Rmax_store(signalpo_n, 1);
    ...
    for ii = 1 : Nite
        ...
        for capa_n = 1 : length(beta_m)
            ...
            for n = 1 : N_inex
                betae_n = elevation_range((signalpo_n-1) * 2 + 1, capa_n) + (elevation_range((signalpo_n-1) * 2 + 2, capa_n) - elevation_range((signalpo_n-1) * 2 +1,  capa_n)) / (N_inex-1) * (n - 1);
                ctemp = zeros(K, 1);
                ano_temp = 0;
                for k = 1 : K
                    Aaz = -min(12 * phi(k, capa_n)^2 / (70/180*pi)^2, 25);
            ...
            betae = elevation_range((signalpo_n-1) * 2 +1, capa_n) + (elevation_range((signalpo_n-1) * 2 +2, capa_n) - elevation_range((signalpo_n-1) * 2 +1, capa_n)) / (N_inex-1) * (capa_index - 1);
            for k = 1 : K
                Aaz = -min(12 * phi(k, capa_n)^2 / (70/180*pi)^2, 25);
...
set(h,'PaperType','A4');
xx = axes('FontSize',16);
semilogy(Rmax_store, capacity(1:length(Rmax_store), 1), 'k--d','LineWidth',2,'MarkerSize',14)
hold on
semilogy(Rmax_store, capacity(1:length(Rmax_store), 3), 'k--^','LineWidth',2,'MarkerSize',14)
semilogy(Rmax_store, capacity(1:length(Rmax_store), 4), 'k-s','LineWidth',2,'MarkerSize',14)

xlim([min(Rmax_store), max(Rmax_store)])
le = legend('fixed mechanical tilt','dynamic electrical tilt', 'the proposed tilts', 'Location', 'southwest');
set(le,'Fontsize',16,'Fontname','Times')
set(gca,'XTick',Rmax_store)
xlabel('Maximal cell radius (meters)','Fontsize',20,'Fontname','Times')
ylabel('Spectrum efficiency (bps/Hz)','Fontsize',20,'Fontname','Times')
grid on
print(h,'-dpdf','capacity_radius')