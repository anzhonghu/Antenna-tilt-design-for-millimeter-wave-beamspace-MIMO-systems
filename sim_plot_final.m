...
Rmax = 100;
sigman2 = 10^(-7.2)*1e-3;
rho_store = [1e0; 1e1; 1e2; 1e3;];%rho/N in the paper
height = 10;
f = 80*10^9;
lambda = 3 * 10^8 / f;
miu = 0.5;
...
Nite = 1e2;
capacity = zeros(length(rho_store), length(beta_m) * 2);
...
set(h,'PaperType','A4');
xx = axes('FontSize',16);
semilogy(10 * log10(rho_store / sigman2), capacity(1:length(rho_store), 1), 'k--d','LineWidth',2,'MarkerSize',14)
hold on
semilogy(10 * log10(rho_store / sigman2), capacity(1:length(rho_store), 3), 'k--^','LineWidth',2,'MarkerSize',14)
semilogy(10 * log10(rho_store / sigman2), capacity(1:length(rho_store), 4), 'k-s','LineWidth',2,'MarkerSize',14)

xlim([min(10 * log10(rho_store / sigman2)), max(10 * log10(rho_store / sigman2))])
...
set(gca,'XTick',10 * log10(rho_store / sigman2))
