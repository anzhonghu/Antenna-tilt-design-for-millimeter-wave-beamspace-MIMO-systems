...
Nite = 1e2;
capacity = zeros(length(Phi3db_store), length(beta_m) * 2);
...
for signalpo_n = 1 : length(Phi3db_store)
    phi3db = Phi3db_store(signalpo_n, 1);
    for ii = 1 : Nite
        pos = zeros(K, 3);
        ...
        for k = 1 : K
            ...
            for capa_n = 1 : length(beta_m)
                ...
                Ael = -min(12 * theta(k, capa_n)^2 / (phi3db/180*pi)^2, 20);
                D0 = -min(-Aaz-Ael, 25);
                D0 = 10^(D0*0.1);
        ...
        for capa_n = 1 : length(beta_m)
            ...
            for n = 1 : N_inex
                ...
                for k = 1 : K
                    Aaz = -min(12 * phi(k, capa_n)^2 / (70/180*pi)^2, 25);
                    Ael = -min(12 * (theta(k, capa_n)-betae_n)^2 / (phi3db/180*pi)^2, 20);
                    D0 = -min(-Aaz-Ael, 25);
                    D0 = 10^(D0*0.1);
            ...
            for k = 1 : K
                Aaz = -min(12 * phi(k, capa_n)^2 / (70/180*pi)^2, 25);
                Ael = -min(12 * (theta(k, capa_n)-betae)^2 / (phi3db/180*pi)^2, 20);
                D0 = -min(-Aaz-Ael, 25);
                D0 = 10^(D0*0.1);
                beta(k, length(beta_m) + capa_n) = sqrt(D0 * lambda^2 / (16 * pi^2 * pos(k, 3)^2)) * beta(k, capa_n) / abs(beta(k, capa_n));
...
set(h,'PaperType','A4');
xx = axes('FontSize',16);
semilogy(Phi3db_store, capacity(1:length(Phi3db_store), 1), 'k--d','LineWidth',2,'MarkerSize',14)
hold on
semilogy(Phi3db_store, capacity(1:length(Phi3db_store), 3), 'k--^','LineWidth',2,'MarkerSize',14)
semilogy(Phi3db_store, capacity(1:length(Phi3db_store), 4), 'k-s','LineWidth',2,'MarkerSize',14)

xlim([min(Phi3db_store), max(Phi3db_store)])
...
set(gca,'XTick',Phi3db_store)
xlabel('HPBW in the elevation pattern (degrees)','Fontsize',20,'Fontname','Times')
ylabel('Spectrum efficiency (bps/Hz)','Fontsize',20,'Fontname','Times')
grid on
print(h,'-dpdf','capacity_3db')